...
export default class AceBinder {
  ...
  constructor(editor, model, collaborative, radarViewElement, port_machine) {
    this._editor = editor;
    ...
    this._port_machine = port_machine;
    this.new_content = false;
    this.control_flow_update();
  }
  ...
  async control_flow_update()
  { while(true)
    { await new Promise (resolve => {setTimeout(() => resolve(this.update_file_ssh()), 1000)});
    }
  }
  ...
  update_file_ssh()
  { if(this.new_content)  
      return fetch('/ssh_handler/file_on_update', {
        method: 'POST',
        headers: {
        ...
        body: JSON.stringify({
          'id_element': this._model.model().modelId(), //id
          'path': this._model.model().root().get('path').value(),
          'port_machine': this._port_machine // Puerto de la máquina
        }),
      })
      .then(() => {this.new_content = false;});
  }
